            ...
            success: function (response) {
                if (response.length > 0) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        ...
                        $elem += '<li onclick="searchGoto(' + response[i]["ID"] + ', \'' + response[i]['Type'] + '\')">';
                        ...
                        $elem += '<a class="user-name" href="#">' + response[i]["Title"] + '</a>';
...
function searchGoto(id, type){ 
    console.log(id + type);
    //location.href = "/" + location.protocol + '//' + location.hostname + (location.port ? ':' + location.port : '') + "Profile/Index/" + id;

    if (type == "U") location.href = "/Profile/Index/" + id;
    else location.href = "/Post/Index/" + id;
